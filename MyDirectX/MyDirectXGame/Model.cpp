...
Model *Model::CreateFromObject(const std::string &text, bool smoothing)
{
	...
	instance->Initialize(text, smoothing);
...
void Model::LoadMaterial(const std::string &directoryPath, const std::string &filename)
{
	...
	while (getline(file, line)) {
		...
		if (key == "map_Kd") {
			// テクスチャのファイル名読み込み
			line_stream >> material->filename;

			// フルパスからファイル名を取り出す
			size_t pos1;
			pos1 = material->filename.rfind('\\');
			if (pos1 != string::npos) {material->filename = material->filename.substr(pos1 + 1, material->filename.size() - pos1 - 1);}
...
{
	const string filename = text + ".obj";
	const string directoryPath = baseDirectory + text + "/";
	...
	while (getline(file, line)) {
		...
		if (key == "f")
		{
			...
			while (getline(line_stream, index_string, ' ')) {
				...
				// マテリアル、テクスチャがある場合
				if (material && material->filename.size() > 0) {
...
void Model::CreateBuffers(ID3D12Device* device)
{
	...
	UINT sizeVB = static_cast<UINT>(sizeof(VertexPosNormalUv) *
			vertices.size());
	...
	const DirectX::Image * ing = scratchIng.GetImage(0, 0, 0);
	assert(ing);
	...
	D3D12_HEAP_FLAG_NONE,
	&texresDesc,
	D3D12_RESOURCE_STATE_GENERIC_READ, // 757+ HIBFE
	nullptr,
