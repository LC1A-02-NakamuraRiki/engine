...
void ParticleManager::InitializeGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources_/shaders/ParticleVS.hlsl",	// シェーダファイル名
		nullptr,
	...
	// 頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources_/shaders/ParticleGS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources_/shaders/ParticlePS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
void ParticleManager::LoadTexture()
{
	...
	result = LoadFromWICFile(
		L"Resources_/effect1.png", WIC_FLAGS_NONE,
		&metadata, scratchImg);
	if (FAILED(result)) {
		assert(0);
	}
...
void ParticleManager::Update()
{
	...
	for (std::forward_list<Particle>::iterator it = particles.begin();
		...
		it->frame++;
		//速度に加速度を加算
		it->velosity = it->velosity + it->accel;
		////速度による移動
		it->position = it->position + it->velosity;
