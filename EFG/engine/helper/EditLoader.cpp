...
const std::string EditLoader::kDefaultBaseDirectory = "Resources/levels/";
const std::string EditLoader::kExtension = ".json";

EditData* EditLoader::LoadFile(const std::string& fileName) {
    //連結してフルパスを得る
	...
	for (nlohmann::json& object : deserialized["objects"]) {
		...
		if (type.compare("MESH") == 0) {
			//要素追加
			editData->objects.emplace_back(EditData::ObjectData{});
			//今追加した要素の参照を得る
			EditData::ObjectData& objectData = editData->objects.back();
			...
			objectData.translation.m128_f32[2] = (float)transform["translation"][0] * -1.0f;
			objectData.translation.m128_f32[3] = 1.0f;
			//回転角
			objectData.rotation.m128_f32[0] = (float)transform["rotation"][1] * -1.0f;
			objectData.rotation.m128_f32[1] = (float)transform["rotation"][2] * -1.0f;
